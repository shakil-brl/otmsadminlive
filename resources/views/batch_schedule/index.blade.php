...
@section('content')
    ...
    <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1 m-5">
        ...
        <li class="breadcrumb-item text-muted">
            <a
                href="{{ route('batch-schedule.batches') }}"class="text-muted text-hover-primary">{{ __('batch-schedule.provider_batch_list') }}</a>
        </li>
        <!--end::Item-->
        <!--begin::Item-->
        <li class="breadcrumb-item">
                                ...
                                @if ($schedule_detail['status'] == 1)
                                    @if ($date <= \Carbon\Carbon::now())
                                        <a id="{{ $schedule_detail['id'] }}" class="btn btn-detail start-class  update"
                                            type="button" data-bs-toggle="modal" data-bs-target="#classStartModal" type="button">
