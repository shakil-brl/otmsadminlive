...
<div class="d-flex justify-content-between align-items-end">
    <h1 class="title">Geoupazila Information</h1>
    <a href="{{ route('geoupazilas.create') }}" class="btn btn-primary">
    ...
    <div class="card-body">
        ...
        <div class="table-responsive">
            <table class="table  table-bordered">
                ...
                <tbody>
                    @foreach ($geoupazilas as $geoupazila)
                    <tr>
                        <td>{{ $loop->iteration}}</td>

                        <td>{{ $geoupazila->Code }}</td>
                        <td>{{ $geoupazila->Name }}</td>
                        <td>{{ $geoupazila->NameEng }}</td>
                        <td>{{ $geoupazila->ParentCode }}</td>

                        <td>
                            <form action="{{ route('geoupazilas.destroy',$geoupazila->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                    ...
                                    <ul class="dropdown-menu">
                                        <li>
                                            <a class="dropdown-item"
                                                href="{{ route('geoupazilas.show',$geoupazila->id) }}">
                                                <span class="material-icons-outlined">
                                                    visibility
                                                </span>
                                                View
                                        ...
                                        <li>
                                            <a class="dropdown-item"
                                                href="{{ route('geoupazilas.edit',$geoupazila->id) }}">
                                                <span class="material-icons-outlined">
                                                    edit
                                                </span>
                                                Edit
