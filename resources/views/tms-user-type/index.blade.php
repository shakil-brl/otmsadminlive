...
<div class="d-flex justify-content-between align-items-end">
    <h1 class="title">Tms User Type Information</h1>
    <a href="{{ route('user-type.create') }}" class="btn btn-primary">
    ...
    <div class="card-body">
        ...
        <div class="table-responsive">
            <table class="table  table-bordered">
                ...
                <tbody>
                    @foreach ($tmsUserTypes as $tmsUserType)
                    <tr>
                        <td>{{ $loop->iteration}}</td>

                        <td>{{ $tmsUserType->role_id }}</td>
                        <td>{{ $tmsUserType->ProfileId }}</td>
                        <td>{{ $tmsUserType->district_id }}</td>
                        <td>{{ $tmsUserType->upazila_id }}</td>
                        <td>{{ $tmsUserType->provider_id }}</td>
                        <td>{{ $tmsUserType->created_user_id }}</td>

                        <td>
                            <form action="{{ route('user-type.destroy',$tmsUserType->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                    ...
                                    <ul class="dropdown-menu">
                                        <li>
                                            <a class="dropdown-item"
                                                href="{{ route('user-type.show',$tmsUserType->id) }}">
                                                <span class="material-icons-outlined">
                                                    visibility
                                                </span>
                                                View
                                        ...
                                        <li>
                                            <a class="dropdown-item"
                                                href="{{ route('user-type.edit',$tmsUserType->id) }}">
                                                <span class="material-icons-outlined">
                                                    edit
                                                </span>
                                                Edit
